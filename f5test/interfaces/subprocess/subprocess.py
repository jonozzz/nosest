...
ur"""subprocess - Subprocesses with accessible I/O streams
...
mswindows = (sys.platform == u"win32")
...
class CalledProcessError(SubprocessError):
    u"""This exception is raised when a process run by check_call() or
...
class TimeoutExpired(SubprocessError):
    ...
    def __str__(self):
        return (u"Command '%s' '%s' timed out after %s seconds" %
                (self.cmd, self.output, self.timeout))
...
#    import pickle
    ...
    try:
        ...
        # warnings.warn(u"The _posixsubprocess module is not being used. "
        #              u"Child process reliability may suffer if your "
        #              u"program uses threads.", RuntimeWarning)

    # When select or poll has indicated that the file is writable,
    # we can write up to _PIPE_BUF bytes without risk of blocking.
    # POSIX defines PIPE_BUF as >= 512.
    _PIPE_BUF = getattr(select, u'PIPE_BUF', 512)

    _FD_CLOEXEC = getattr(fcntl, u'FD_CLOEXEC', 1)
    ...
    if _posixsubprocess and hasattr(_posixsubprocess, 'cloexec_pipe'):
...
__all__ = [u"Popen", u"PIPE", u"STDOUT", u"call", u"check_call", u"getstatusoutput",
...
def call(*popenargs, **kwargs):
    if 'timeout' in kwargs:
        ...
        timeout = None
    u"""Run command with arguments.  Wait for command to complete or
...
def check_call(*popenargs, **kwargs):
    u"""Run command with arguments.  Wait for command to complete.  If
...
def check_output(*popenargs, **kwargs):
    if 'timeout' in kwargs:
        ...
        timeout = None
    ur"""Run command with arguments and return its output as a byte string.
    ...
    """
    if u'stdout' in kwargs:
        raise ValueError(u'stdout argument not allowed, it will be overridden.')
...
def getoutput(cmd):
    u"""Return output (stdout or stderr) of executing cmd in a shell.
...
class Popen(object):
    def __init__(self, args, bufsize=0, executable=None,
                 stdin=None, stdout=None, stderr=None,
                 preexec_fn=os.setpgrp, close_fds=_PLATFORM_DEFAULT_CLOSE_FDS,
                 shell=False, cwd=None, env=None, universal_newlines=False,
                 startupinfo=None, creationflags=0,
                 restore_signals=True, start_new_session=False,
                 pass_fds=()):
        u"""Create new Popen instance."""
        _cleanup()
        ...
        if not isinstance(bufsize, int):
            raise TypeError(u"bufsize must be an integer")

        if mswindows:
            if preexec_fn is not None:
                raise ValueError(u"preexec_fn is not supported on Windows "
                                 u"platforms")
            ...
            if close_fds is _PLATFORM_DEFAULT_CLOSE_FDS:
                ...
                raise ValueError(
                        u"close_fds is not supported on Windows platforms"
                        u" if you redirect stdin/stdout/stderr")
        else:
            # POSIX
            if close_fds is _PLATFORM_DEFAULT_CLOSE_FDS:
                close_fds = True
            if pass_fds and not close_fds:
                warnings.warn(u"pass_fds overriding close_fds.", RuntimeWarning)
                close_fds = True
            if startupinfo is not None:
                raise ValueError(u"startupinfo is only supported on Windows "
                                 u"platforms")
            if creationflags != 0:
                raise ValueError(u"creationflags is only supported on Windows "
                                 u"platforms")
        ...
        if c2pread != -1:
            self.stdout = io.open(c2pread, u'rb', bufsize)
            if universal_newlines:
                self.stdout = io.TextIOWrapper(self.stdout)
        if errread != -1:
            self.stderr = io.open(errread, u'rb', bufsize)
            if universal_newlines:
        ...
        try:
            ...
            for f in ifilter(None, [self.stdin, self.stdout, self.stderr]):
                try:
                    f.close()
    ...
    def _get_devnull(self):
        if not hasattr(self, u'_devnull'):
    ...
    def communicate(self, input=None, timeout=None):  # @ReservedAssignment
        ...
        if self._communication_started and input:
            raise ValueError(u"Cannot send input after starting communication")
    ...
    def _remaining_time(self, endtime):
        u"""Convenience for _communicate when computing timeouts."""
        if endtime is None:
            return None
        else:
    ...
    def _check_timeout(self, endtime, orig_timeout):
        u"""Convenience for checking if a timeout has expired."""
        if endtime is None:
            return
    ...
    if mswindows:
        ...
        def _get_handles(self, stdin, stdout, stderr):
            u"""Construct and return tuple with IO objects:
        ...
        def _find_w9xpopen(self):
            ...
            if not os.path.exists(w9xpopen):
                ...
                w9xpopen = os.path.join(os.path.dirname(sys.exec_prefix),
                                        u"w9xpopen.exe")
                if not os.path.exists(w9xpopen):
                    raise RuntimeError(u"Cannot locate w9xpopen.exe, which is "
                                       u"needed for Popen to work with your "
                                       u"shell or platform.")
            return w9xpopen
        ...
        def _execute_child(self, args, executable, preexec_fn, close_fds,
            ...
            u"""Execute program (MS Windows version)"""

            assert not pass_fds, u"pass_fds not supported on Windows."
        ...
        def wait(self, timeout=None, endtime=None):
            u"""Wait for child process to terminate.  Returns returncode
            attribute."""
        ...
        def _communicate(self, input, endtime, orig_timeout):  # @ReservedAssignment
            ...
            if self.stderr and not hasattr(self, u"_stderr_buff"):
                self._stderr_buff = []
                self.stderr_thread = \
                        threading.Thread(target=self._readerthread,
        ...
        def send_signal(self, sig):
            u"""Send a signal to the process
            """
        ...
        def _execute_child(self, args, executable, preexec_fn, close_fds,
                    ...
                    if _posixsubprocess:
                        ...
                        if self.pid == 0:
                            ...
                            try:
                                ...
                                if restore_signals:
                                    signals = (u'SIGPIPE', u'SIGXFZ', u'SIGXFSZ')
                                    for sig in signals:
                                        if hasattr(signal, sig):
                                            signal.signal(getattr(signal, sig),
                                                          signal.SIG_DFL)

                                if start_new_session and hasattr(os, u'setsid'):
                                    os.setsid()
                                ...
                                try:
                                    ...
                                    if isinstance(exc_value, OSError):
                                        ...
                                        errno_num = 0
                                    message = u'%s:%x:%s' % (exc_type.__name__,
                                                            errno_num, exc_value)
                                    message = message.encode(errors=u"surrogatepass")
            ...
            if data:
                ...
                try:
                    ...
                    print u'Bad exception data:', repr(data)
                    exception_name = 'RuntimeError'
                    hex_errno = '0'
                    err_msg = 'Unknown'
                child_exception_type = getattr(
                        __builtin__, exception_name.decode(u'ascii'),
                        RuntimeError)
                for fd in (p2cwrite, c2pread, errread):
                    if fd != -1:
                        os.close(fd)
                err_msg = err_msg.decode(errors=u"surrogatepass")
                if issubclass(child_exception_type, OSError) and hex_errno:
                    errno_num = int(hex_errno, 16)
                    if errno_num != 0:
                        err_msg = os.strerror(errno_num)
                        if errno_num == errno.ENOENT:
                            err_msg += u': ' + repr(args[0])
        ...
        def _handle_exitstatus(self, sts, _WIFSIGNALED=os.WIFSIGNALED,
            ...
            if _WIFSIGNALED(sts):
                ...
                # Should never happen
                raise RuntimeError(u"Unknown child exit status!")
        ...
        def terminate(self):
            u"""Terminate the process with SIGTERM
            """
            self.send_signal(signal.SIGTERM)

        def kill(self):
            u"""Kill the process with SIGKILL
            """
