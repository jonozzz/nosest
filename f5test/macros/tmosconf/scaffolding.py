    ...
    for folder in chain:
        if klass:
            if isinstance(klass, collections.Iterable):
                keys = []
                for k in klass:
                    keys += [x for x in list(folder.content_map.keys()) if issubclass(x, k)]
            else:
                keys = [x for x in list(folder.content_map.keys()) if issubclass(x, klass)]
        else:
            keys = list(folder.content_map.keys())
...
class Stamp(object):
    ...
    def get_full_path(self):
        if self.folder is None:
            ...
            return self.folder.SEPARATOR.join((self.folder.key(), str(self.name)))
